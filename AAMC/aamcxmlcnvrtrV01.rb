...
Dir["**/*.xml"].each do |f| # Runs on each xml file in current folder and its subdirectories
	...
	unless @doc.xpath('//@discipline')
	 ...
	 # run on each section found in books/book in XML
			...
			section.xpath(ITEM_HANDLERS).each do |item| # run on each item with certain handler value(s)
				...
				answer_html 			||= item.xpath('value[@name="overallrejoinder"]').first&.children&.to_html # take first rejoinder as answer explanation - assumes all rejoinders identical
