...
### CONFIGURATIONS
ROOT_XPATH	= '/book/book-back/book-part/body/book-part/body/question-wrap-group/question-wrap'.freeze
...
Dir["**/*.xml"].each do |f| # Runs on each xml file in current folder and its subdirectories
	...
	CSV.open("#{f.chomp(".xml")}.csv", 'w') do |csv| # new CSV with same name as current file
		...
		@doc.xpath(ROOT_XPATH).each do |section| # run on each section found in books/book in XML
			...
			section.xpath('question').each do |item| # run on each item with certain handler value(s)
				...
				question = item.xpath('p') # populate question stem to use within question_html
				choices = item.xpath('option/p')
				...
				question_category 		= item.xpath('@content-type')
				...
				question_type			||= "SMC"
				choices_count			||= "4" #placeholder in case not provided
				...
				csv << [ # makes a new row corresponding to current XML item
					...
					'', # cleans up HTML to populate answer_content_file
					# exam_name, # exam_name
					'', # exam_name
					'', # exam_section_name
